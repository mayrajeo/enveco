...
__all__ = ['plot_point_cloud', 'plot_2d_views', 'las_to_df', 'mask_plot_from_lidar', 'height_metrics', 'z_stats',
           'z_percentages', 'z_quantiles', 'z_cumul', 'z_stat_cols', 'z_pct_cols', 'z_quant_cols', 'z_cumul_cols',
           'z_cols', 'intensity_metrics', 'i_stats', 'i_cumul_zq', 'i_stat_cols', 'i_qumul_zq_cols', 'i_cols',
           'class_metrics', 'class_cols', 'density_metrics', 'density_cols', 'point_cloud_metrics',
           'point_cloud_metric_cols', 'VoxelImage', 'get_las_data', 'get_3d_grid', 'VoxelBlock', 'LasColReader',
           'VoxelDataLoaders']
...
def z_stats(lasfile:pd.DataFrame, min_h:float=1.5) -> list:
    ...
    zskew = skew(vege.z)
    zkurt = kurtosis(vege.z)
    zentropy = entropy(vege.z)
...
def z_cumul(lasfile:pd.DataFrame, min_h:float=1.5) -> list:
    "Calculate `zpcumx` for x in 1...9"
    vege = lasfile[lasfile.z > min_h]
    intervals = np.linspace(min_h, vege.z.max(), 11)
    cum_percentages = [len(vege.z[vege.z <= interv])/len(vege) for interv in intervals]
    return cum_percentages[1:10]

z_stat_cols = ['zmax', 'zmean', 'zsd', 'zskew', 'skurt', 'zentropy']
z_pct_cols = ['pzabovemean'] + [f'pzabove{x}' for x in range(1,10)]
z_quant_cols = [f'zq{x}' for x in np.linspace(0.05,95, 19)]
...
def class_metrics(lasfile:pd.DataFrame, min_h:int=1.5) -> list:
    ...
    ground = (len(lasfile)- len(vege_df)) / len(lasfile)
...
class_cols = ['vege', 'veg_ground_ratio']

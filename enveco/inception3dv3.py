...
__all__ = ['BasicConv3d', 'calc_same_padding', 'Inception3dV3', 'Inception3dV3Outputs', 'Inception3dA', 'Inception3dB',
           'Inception3dC', 'Inception3dD', 'Inception3dE']
...
class BasicConv3d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.same_padding:
            x = F.pad(x, (calc_same_padding(x.shape, self.kernel, self.stride)))
        x = self.conv(x)
        x = self.bn(x)
        return F.relu(x, inplace=True)

def calc_same_padding(inshape, kernel, strides):
    # [left, right, top, bot, front, back] is the order for F.pad so calculate it
...
class Inception3dV3(nn.Module):
    "InceptionV3 but for volumetric data. Modified from torchvision.models"
...
class Inception3dA(nn.Module):
    "Inception_layer1 in tf implementation?"
    def __init__(
        self,
        in_channels:int,
        outshapes:int,
    ...
    def _forward(self, x:Tensor) -> List[Tensor]:
        ...
        branch_a = self.branch_a_1(branch_a)
        branch_a = self.branch_a_2(x)

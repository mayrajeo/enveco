...
__all__ = ['plot_point_cloud', 'plot_2d_views', 'las_to_df', 'mask_plot_from_lidar', 'calc_height_features',
           'calc_height_quantiles', 'calc_point_features', 'calc_intensity_features', 'calc_canopy_densities',
           'calc_point_proportions', 'height_cols', 'point_cols', 'intensity_cols', 'quantile_cols', 'proportion_cols',
...
def calc_height_features(lidar_df:pd.DataFrame, min_h:int=1.5) -> list:
    "Calculate height based features from dataframe, features 1, 2 and 4 from earlier"
...
def calc_point_proportions(lidar_df:pd.DataFrame, min_h:int=1.5) -> list:
    "Calculate point proportions, feature 7"
    vege_df = lidar_df[lidar_df.z >= min_h]
    if len(vege_df) == 0: return None
    quantiles = [vege_df.z.quantile(q) for q in np.linspace(0,.95,20)]
...
proportion_cols = [f'p{int(q):02d}' for q in np.linspace(0,95, 20)]

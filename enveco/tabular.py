...
__all__ = ['get_lidar_feature', 'get_image_procs', 'EnvecoPreprocessor', 'RegressionInterpretation',
           'plot_sklearn_regression', 'adjusted_R2Score', 'rrmse', 'bias', 'bias_pct', 'ANNEnsemble']
...
class EnvecoPreprocessor():
    ...
    def preprocess_lidar(self, target_col, path, min_h:float=1.5, mask_plot:bool=True, height_features:bool=True,
                         ...
                         point_proportions:bool=True) -> Tuple[TabularPandas, TabularPandas]:
        ...
        trainval_tb = TabularPandas(trainval, procs=[Normalize.from_stats(*norm_stats)],
                                   cont_names=feature_cols, y_names=target_col,
                                   splits=ColSplitter(col='is_valid')(trainval))
        test_tb = TabularPandas(test, procs=[Normalize.from_stats(*norm_stats)],
                                cont_names=feature_cols, y_names=target_col)
...
class ANNEnsemble():
    ...
    def __init__(self, dls, y_range:tuple, metrics, n_models=10, **learner_kwargs):
        "Create an ensemble of ANN models. TODO add option to use different parameters"
        self.dls = dls
        self.metrics = metrics
        self.models = [tabular_learner(dls, metrics=metrics, y_range=y_range, **learner_kwargs) for _ in range(n_models)]

...
__all__ = ['open_geotiff', 'circular_mask', 'calc_normalized_spectral_index', 'calc_avi', 'calc_savi', 'calc_gci',
           'calc_metrics']
...
def circular_mask(data:np.ndarray, radius:float=31) -> np.ndarray:
    "Select only data from within field plot"
...
def calc_normalized_spectral_index(im:np.ndarray, band_1:int, band_2:int) -> np.ndarray:
    "Calculate normalized spectral index (band_1 - band_2)/(band_1-band_2). Can be used with NDVI and such"
...
def calc_metrics(image:np.ndarray) -> list:
